...
const app = express();
// デプロイナウが割り当てるポート番号（指定がなければ 3000）を使用
const PORT = process.env.PORT || 3000;

app.use(cors());
app.use(express.json());

// 静的ファイル（index.html, logo.png等）をルートディレクトリから配信
...
app.post('/api/gemini', async (req, res) => {
  ...
  if (!apiKey) {
    return res.status(500).json({ 
      error: 'GEMINI_API_KEY がデプロイナウの環境変数に設定されていません。管理画面で GEMINI_API_KEY を登録してください。' 
    });
  }
...
// サーバー起動
app.listen(PORT, () => {
  console.log(`Song Ripple Server is running on port ${PORT}`);
});
